...
describe 'forecast endpoint', :vcr do
  it 'retrieves weather for a city' do
    ...
    expect(weather[:data][:attributes][:daily_weather].first).to have_key(:sunset)
    expect(weather[:data][:attributes][:daily_weather].first[:sunset]).to be_a String
